...
describe('Authorization - Mastercard Payloads', () => {
  ...
  // Filter for authorization + mastercard
  const filteredRows = rows.filter(
    (row) =>
      (row['transaction_type'] || '').toLowerCase() === 'authorization' &&
      (row['card_type'] || '').toLowerCase() === 'mastercard'
  );
  ...
  filteredRows.forEach((row) => {
    ...
    test(`Validate JSON for order_number ${orderNumber}`, () => {
      ...
      Object.keys(row).forEach((key) => {
        let testKey = key;
        if (key === 'transaction_amount') testKey = 'amount';
